--[[ 
    DVN HUB - CUSTOM UI [FINAL V3]
    Features: 
    - Resizable Window (Working)
    - Fixed Sidebar (No Scroll)
    - Stable Content Loading (Fixes Empty Page Bug)
...
for index, tabName in ipairs(Tabs) do
    ...
    -- Perbaikan: Matikan AutomaticCanvasSize, gunakan script update manual di bawah
    ...
    -- SCRIPT PENTING: Update ukuran scroll manual agar konten muncul
...
-- 6. HELPER FUNCTIONS (Dengan Auto Layout Order)
local function GetNextOrder(parent)
    local maxOrder = 0
    for _, child in ipairs(parent:GetChildren()) do
        if child:IsA("GuiObject") and child:FindFirstChild("LayoutOrder") then
            if child.LayoutOrder > maxOrder then maxOrder = child.LayoutOrder end
        end
    end
    return maxOrder + 1
end
...
InfoLabel.LayoutOrder = GetNextOrder(TabFrames["Info"])
...
-- [TELEPORT]
local IslandCoords = {
    ["Christmas Island"] = Vector3.new(100, 50, 100), 
...
CreateButton(TabFrames["Teleport"], "Teleport Now", function()
    if selectedLocation and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
        ...
        -- Implementasikan CFrame Teleport di sini
    end
end)
...
-- 8. LOGIKA: DRAG, MINIMIZE, RESIZE (FIXED & LENGKAP)
...
MinBtn.MouseButton1Click:Connect(function()
    if isMinimized then
        ...
        MinBtn.Text = "-"
        if MainFrame:FindFirstChild("ResizeBtn") then MainFrame.ResizeBtn.Visible = true end
    else
        -- Minimize
        currentSize = MainFrame.Size -- Simpan size terakhir
        TweenService:Create(MainFrame, TweenInfo.new(0.4, Enum.EasingStyle.Quart), {Size = UDim2.new(0, currentSize.X.Offset, 0, 35)}):Play()
        MinBtn.Text = "+"
        if MainFrame:FindFirstChild("ResizeBtn") then MainFrame.ResizeBtn.Visible = false end
    end
...
ResizeBtn.Image = "rbxassetid://3599185146" -- Ikon Panah Resize
ResizeBtn.ImageColor3 = TEXT_INACTIVE
...
UserInputService.InputChanged:Connect(function(input)
    if resizing and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
        ...
        local newWidth = math.max(MIN_SIZE.X, startSize.X.Offset + delta.X)
        local newHeight = math.max(MIN_SIZE.Y, startSize.Y.Offset + delta.Y)
