...
<<<<<<< HEAD
    ...
    local success, res = pcall(function()
        return req({ Url = SETTINGS.WebhookURL, Method = "POST", Headers = { ["Content-Type"] = "application/json" }, Body = HttpService:JSONEncode(payload) })
    end)
    if not success then
        warn("[Webhook] Error Script:", res)
    elseif res and res.StatusCode and (res.StatusCode < 200 or res.StatusCode >= 300) then
        warn("[Webhook] Gagal Kirim. Status:", res.StatusCode, "Body:", res.Body)
    elseif success then
