...
local WEBHOOK_AVATAR = "https://cdn.discordapp.com/attachments/1452679772428828827/1464646946575749181/IMG_5096.png?ex=69763a43&is=6974e8c3&hm=7e809899697e3139b6d420ad86f3fd9e99fd64caec97a029e3df5efe17cd0bcd&"
...
local FOCUS_FISH = {
    ...
    ["Eggy Enchant Stone"]    = { Enabled = true,  Color = 0xFF1919 }
}
...
local function loadFishDataLegacy()
    pcall(function()
        ...
        local count = 0
        if not itemsFolder then return warn("CIA Logger: Items folder not found.") end
        ...
        print("🐟 CIA Logger: Loaded " .. count .. " fish images.")
    end)
end
...
local function loadFishData()
    ...
    if itemsFolder then
        ...
        warn("CIA Logger: Items folder not found, trying full ReplicatedStorage scan.")
    end
    ...
    print("ðŸŸ CIA Logger: Loaded " .. count .. " fish images.")
end
...
local function sendJoinLeave(player, joined)
    ...
    send({ username = WEBHOOK_NAME, avatar_url = WEBHOOK_AVATAR, embeds = {{
        ...
        footer = { text = "Ciaa  •  Server Activity" },
        timestamp = os.date("!%Y-%m-%dT%H:%M:%SZ")
    }} })
end
...
-- CLEANUP OLD GUI
if GUI_PARENT:FindFirstChild("CIA_HUB_LOGGER") then
    GUI_PARENT.CIA_HUB_LOGGER:Destroy()
end

local ScreenGui = Instance.new("ScreenGui")
ScreenGui.Name = "CIA_HUB_LOGGER"
...
local MAIN_BG = Color3.fromRGB(255, 240, 245) -- Lavender Blush
local ELEMENT_BG = Color3.fromRGB(255, 228, 225) -- Misty Rose
local ACCENT_COLOR = Color3.fromRGB(255, 105, 180) -- Hot Pink
local TEXT_COLOR = Color3.fromRGB(100, 50, 80) -- Dark Pink/Purple
local TEXT_DIM = Color3.fromRGB(180, 130, 150) -- Dim Pink
...
MainGradient.Color = ColorSequence.new{ ColorSequenceKeypoint.new(0.0, Color3.fromRGB(255, 230, 240)), ColorSequenceKeypoint.new(1.0, Color3.fromRGB(255, 182, 193)) }
...
local Title = Instance.new("TextLabel", Header); Title.Text = "CIA LOGGER"; Title.Size = UDim2.new(0, 200, 1, 0); Title.Position = UDim2.new(0, 12, 0, 0); Title.BackgroundTransparency = 1; Title.TextColor3 = TEXT_COLOR; Title.Font = Enum.Font.GothamBold; Title.TextSize = 16; Title.TextXAlignment = Enum.TextXAlignment.Left
...
for i, name in ipairs(Tabs) do
    ...
    TabFrames[name] = Page
    local Btn = Instance.new("TextButton", Sidebar); Btn.Name = name; Btn.LayoutOrder = i; Btn.Size = UDim2.new(1, -16, 0, 28); Btn.BackgroundColor3 = ACCENT_COLOR; Btn.BackgroundTransparency = 1; Btn.Text = name; Btn.Font = Enum.Font.GothamBold; Btn.TextSize = 14; Btn.TextColor3 = TEXT_DIM
...
function CreateToggle(parent, text, defaultVal, callback)
    ...
    local ToggleBtn = Instance.new("TextButton", Frame); ToggleBtn.Size = UDim2.new(0, 36, 0, 18); ToggleBtn.Position = UDim2.new(1, -42, 0.5, -9); ToggleBtn.BackgroundColor3 = defaultVal and ACCENT_COLOR or Color3.fromRGB(200, 180, 190); ToggleBtn.Text = ""
    local TCorner = Instance.new("UICorner", ToggleBtn); TCorner.CornerRadius = UDim.new(1, 0)
    local Dot = Instance.new("Frame", ToggleBtn); Dot.Size = UDim2.new(0, 14, 0, 14); Dot.Position = defaultVal and UDim2.new(1, -16, 0.5, -7) or UDim2.new(0, 2, 0.5, -7); Dot.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
    local DCorner = Instance.new("UICorner", Dot); DCorner.CornerRadius = UDim.new(1, 0)
    local on = defaultVal
    ToggleBtn.MouseButton1Click:Connect(function()
        on = not on
        if on then TweenService:Create(ToggleBtn, TweenInfo.new(0.2), {BackgroundColor3 = ACCENT_COLOR}):Play(); TweenService:Create(Dot, TweenInfo.new(0.2), {Position = UDim2.new(1, -16, 0.5, -7)}):Play()
        else TweenService:Create(ToggleBtn, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(200, 180, 190)}):Play(); TweenService:Create(Dot, TweenInfo.new(0.2), {Position = UDim2.new(0, 2, 0.5, -7)}):Play() end
        pcall(callback, on)
    end)
end
